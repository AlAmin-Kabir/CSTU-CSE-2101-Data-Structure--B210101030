...
struct node *insert_after_a_given_node(struct node *tail,int node,int data)
{
    struct node *ptr,*temp = malloc(sizeof(struct node));
    temp->prev = NULL;
    temp->data = data;
...
int main()
{
    ...
    printf("Enter a node : \n");
    scanf("%d",&node);
    printf("Enter an element to insert after this given node : \n");
    scanf("%d",&data);
    tail = insert_after_a_given_node(tail,node,data);
